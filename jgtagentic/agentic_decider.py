...
class DecisionResult(dict):
    """Dictionary-like decision result with legacy containment behavior."""

    def __contains__(self, item: object) -> bool:  # type: ignore[override]
        return isinstance(item, str) and item in self.get("decision", "")

    def __str__(self) -> str:  # pragma: no cover - used for prints
...
class AgenticDecider:
    ...
    def decide(self, signal: Dict):
        ...
        context = {
            "signal_quality": (
                "High"
                if all([has_fdb, has_alligator, has_volume])
                else "Medium" if any([has_fdb, has_alligator]) else "Low"
            ),
        ...
        decision = DecisionResult(
            {
                "decision": f"Decision for {instrument} {timeframe} - {direction}",
    ...
    def _get_confirmation_level(self, signal: Dict) -> str:
        ...
        confirmations = 0
        if signal.get("fdb_signals"):
            confirmations += 1
        if signal.get("alligator_signals"):
            confirmations += 1
        if signal.get("volume_signals"):
            confirmations += 1
        if signal.get("higher_tf_bias"):
            confirmations += 1
